...
export default async function AdminPage({
  ...
  const [t, authed] = await Promise.all([getTranslations({ locale }), isAuthed()]);

  return (
    <>
      <SiteHeader />
      <main className="mx-auto max-w-3xl px-4 py-12">
        <h1 className="mb-8 text-3xl font-semibold tracking-tight">{t("admin.title")}</h1>
        {authed ? <QueueData locale={locale} /> : <SignIn />}
      </main>
    </>
  );
}

async function QueueData({ locale }: { locale: string }) {
  ...
  const { data: currentUnits } = statUnitIds.length
    ? await db.from("unit").select("id, stats").in("id", statUnitIds)
    : { data: [] };
